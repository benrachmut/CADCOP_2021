...
public class MaxSumStandardFunction extends AgentFunction {
	 ...
	 * initiation of the algorithm (iteration = 0) - Checked !!!
	 */
	...
	// OmerP - will get the message and update context in HashMap - Need to check
	// during run.
	@Override
	...
	protected double[] produceFunctionMessage(NodeId to) {
		...
		if (getOtherNodeIdMessage(to) != null) { // If the message that the other variable node sent is not Null.
			...
			constraintMatrix = addTableToMatrix(getOtherNodeIdMessage(to), constraintMatrix); // Will add the second
	...
	protected double[] getOtherNodeIdMessage(NodeId to) {
		...
		if (to.equals(temp[0])) {
			...
			return variableMsgs.get(temp[1]).getContext(); // Will return the message that received from the other
															// variable node that was received.
		}

		else {

			return variableMsgs.get(temp[0]).getContext(); // Will return the message that received from the other
															// variable node that was received.
	...
	// OmerP - Parameter that are needed to be initialized every time a new problem
	// is initialized.
	@Override
	protected void resetAgentGivenParameters() {
		...
		for (NodeId i : variableMsgs.keySet()) {
			...
			variableMsgs.replace(i, variableMsgs.get(i), null); // OmerP - will put null instead of the value that was
																// stored.
		...
		for (NodeId i : storedMessgesTable.keySet()) {
			...
			storedMessgesTable.replace(i, storedMessgesTable.get(i), null); // OmerP - will put null instead of the
	...
	protected void printStoredMessage(MsgAlgorithmFactor msg) {
		...
		System.out.println("FunctionNode:(" + msg.getSenderId().getId1() + "," + msg.getSenderId().getId2()
				+ ") STORED a message for VariableNode ("
	...
	protected void printSentMessage(MsgAlgorithmFactor msg) {
		...
		System.out.println("Computation Counter:(" + this.computationCounter + "),FunctionNode:("
				+ msg.getSenderId().getId1() + "," + msg.getSenderId().getId2() + ") SENT a message for VariableNode ("
	...
	protected void printReceivedMessage(MsgAlgorithmFactor msg) {
		...
		System.out.println(
				"Computation Counter:(" + this.computationCounter + "), FunctionNode:(" + msg.getRecieverId().getId1()
						+ "," + msg.getRecieverId().getId2() + ") RECEIVED a message from VariableNode ("
	...
	// OmerP - A method to check that the creation of the transpose matrix went
	// correctly.
	...
	protected void printConstraints(NodeId av, double[][] constraints) {
		...
		System.out.println("Function node (" + this.getNodeId().getId1() + "," + this.getNodeId().getId2() + ") "
				+ "constraint matrix with (" + av.getId1() + "," + av.getId2() + ") is "
				+ Arrays.deepToString(constraints) + ".\n");
	...
	protected void printIncreaseInAtomicCounter() {
		...
		System.out.println("FunctionNode:(" + this.getNodeId().getId1() + "," + this.getNodeId().getId2()
				+ ") increased its atomic operation to(" + this.atomicActionCounter + ").\n");
